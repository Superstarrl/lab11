...
public class Pathfinding : MonoBehaviour
{
    ...
    */
    ...
    private int[,] grid; //new grid for use in random function

    [Header("Grid Creation Variables")]
    public int width;
    public int height;
    ...
    private void Start()
    {
        GenerateRandomGrid(height, width, obstacleProbability);
        FindPath(start, goal);
    }
    ...
    {
        if (!Application.isPlaying) return; //make gizmos only draw at runtime since that is when grid is created
    ...
    void GenerateRandomGrid(int width, int height, float obstacleProbability)
    {
        ...
        for (int y = 0; y < grid.GetLength(0); y++)
        {
            for (int x = 0; x < grid.GetLength(1); x++)
            {
                grid[y,x] = Random.Range(0f, 1f) >= obstacleProbability ? 0 : 1;
            }
        }
    }
}
